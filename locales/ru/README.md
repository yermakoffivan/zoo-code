...
## Что нового в v3.66.0

- **Поддержка Claude Sonnet 5** — новейшая модель Claude теперь доступна у провайдеров Anthropic, Bedrock и Vertex
- **Улучшения индексации кода в Semble v0.4.1** — плоский разбор результатов и локализованные сообщения о статусе делают результаты индексации и обратную связь по статусу надёжнее
- **Исправления надёжности задач и делегирования** — стражи переходов состояния, согласование делегирования при запуске и атомарное повторное открытие родительской задачи уменьшают количество недопустимых изменений состояния задач и проблем при передаче делегирования
