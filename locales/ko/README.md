...
- **Semble v0.4.1 코드 인덱싱 개선** — 평탄화된 결과 파싱과 현지화된 상태 메시지로 인덱싱 결과와 상태 피드백의 신뢰성이 높아졌습니다
- **태스크 및 위임 안정성 수정** — 상태 전환 가드, 시작 시 위임 조정, 그리고 부모 태스크의 원자적 재개방으로 잘못된 태스크 상태 변경과 위임 인계 문제를 줄입니다
