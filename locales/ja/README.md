...
- **Semble v0.4.1 のコードインデックス改善** — フラット化された結果解析とローカライズされたステータスメッセージにより、インデックス結果とステータスフィードバックの信頼性が向上しました
- **タスクと委任の信頼性修正** — 状態遷移ガード、起動時の委任整合、および親タスクのアトミックな再オープンにより、無効なタスク状態変更と委任引き継ぎの問題を減らします
